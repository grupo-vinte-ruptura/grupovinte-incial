...
if st.session_state.step == 3:
    ...
    with st.form("analysis_focus_form"):
        ...
        st.info("Selecione as colunas que você acredita serem as mais importantes para prever o problema.")
...
if st.session_state.step == 5:
    ...
    # O método to_csv() é perfeito para isso. index=False evita que os números das linhas sejam incluídos.
    data_string = st.session_state.df.to_csv(index=False)
    
    # Montando o prompt final com as novas instruções
    prompt_final = f"""**Persona:**
Você é um cientista de dados sênior com vasta experiência em modelagem preditiva para a indústria pesada (siderurgia e metalurgia). Sua tarefa é analisar os dados fornecidos e apresentar os resultados de forma clara e visual.
...
- **Análise de Padrões Históricos:** Investigue os seguintes padrões nos dados históricos: {', '.join(st.session_state.get("padroes", []))}.
...
- **Visualização Gráfica (Resultado Principal):** Com base nos resultados, **gere um gráfico de barras** que mostre a importância de cada variável (feature importance) para as previsões do modelo. O gráfico deve ser claro, com títulos nos eixos e valores legíveis.
